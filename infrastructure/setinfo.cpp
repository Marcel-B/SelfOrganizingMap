...
SetInfo SetInfo::SaveSetInfo(std::ofstream &stream) {
  ...
  stream.write((char*)&this->features_size, sizeof(this->features_size));
  for(size_t i = 0; i < this->features_size; ++i){
    std::string name = this->features[i].name;
    size_t name_size = name.size();
    auto cc = name.c_str();
    stream.write((char*)&name_size, sizeof(size_t));
    stream.write(cc, name_size);
    stream.write((char*)&this->features[i].min, sizeof(this->features[i].min));
    stream.write((char*)&this->features[i].max, sizeof(this->features[i].max));
  }
  return *this;
}
...
SetInfo SetInfo::OpenSetInfo(std::ifstream &stream) {
  SetInfo info;
  stream.read((char*)&info.features_size, sizeof(info.features_size));
  ...
  for(size_t i = 0; i < info.features_size; ++i){
    ...
    delete tp;
    stream.read((char*)&info.features[i].min, sizeof(info.features[i].min));
    stream.read((char*)&info.features[i].max, sizeof(info.features[i].max));
  }
  return info;
}
